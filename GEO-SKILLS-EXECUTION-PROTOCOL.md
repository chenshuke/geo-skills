...
- 主线序号为 00-12，其中 00A 是新手一键入口、04A 是关键词池/状态机；90 是故障排查；91 是技能自进化。
- 详细顺序见 `GEO-STUDENT-WORKFLOW.md`。

## 0.1 新手一键入口规则

当用户说“我是新学员，帮我从 0 跑一个 GEO 项目”或类似表达时，优先使用 `geo-student-workflow`，按“项目资料 → 品牌诊断 → 关键词池 → 内容 → 发布 → 收录 → 引用源 → 复盘报告”陪跑。不要一开始向新手展示大量脚本参数；每个阶段只输出当前阶段、已完成、证据、是否需要确认、下一步。
