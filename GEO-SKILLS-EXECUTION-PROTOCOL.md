...
- 主线序号为 00-12，其中 00A 是课程一键入口（三模式）、04A 是关键词池/状态机；90 是故障排查；91 是技能自进化。
- 详细顺序见 `GEO-STUDENT-WORKFLOW.md`。

## 0.1 新手一键入口规则

当用户说“我是新学员/私教班/大师班/20个问题上榜/持续运营飞轮”等类似表达时，优先使用 `geo-student-workflow`，先选择三种模式：快速闭环模式、20问题上榜战役模式、持续运营飞轮模式。不要一开始向新手展示大量脚本参数；每个阶段只输出当前模式、当前阶段、已完成、证据、是否需要确认、下一步。
...
| 技能进化记录 | `skill-evolution` | `00_项目概览/技能进化/` |
...
node geo-source-assets/scripts/source_assets.js --action import --project-dir "项目_品牌GEO" --answers-json answers.json --owned-domains "example.com" --owned-brands "示例品牌A"
```
