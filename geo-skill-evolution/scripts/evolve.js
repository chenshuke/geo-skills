...
function readEvidence(files) {
  return files.map(file => {
    ...
    return { file, exists: fs.existsSync(abs), excerpt };
  });
}
...
function classify(ctx) {
  const hay = `${ctx.symptom}\n${ctx.evidence.map(e => e.excerpt).join('\n')}`;
  const out = { type: 'workflow-pattern', targetSkills: [], severity: ctx.severity || '' };
  if (includesAny(hay, ['openkey','referer','companyid','productid','配置','认证','401'])) { out.type = 'troubleshooting-rule'; out.targetSkills = ['geo-config','geo-runtime','geo-troubleshooter']; }
  else if (includesAny(hay, ['发布','publication','publishedurl','publishurl','posturl','platformurl','人工处理'])) { out.type = 'script-fix'; out.targetSkills = ['geo-publish','geo-indexing','geo-troubleshooter']; }
  else if (includesAny(hay, ['收录','answers','matrix','searchedsites','articleindexed'])) { out.type = 'script-fix'; out.targetSkills = ['geo-indexing','geo-source-assets','geo-analysis']; }
  else if (includesAny(hay, ['上传','upload_article','summaries','封面','oss','图片'])) { out.type = 'script-fix'; out.targetSkills = ['geo-article','geo-content-production']; }
  else if (includesAny(hay, ['竞品','引用源','信源','source asset'])) { out.type = 'industry-playbook'; out.targetSkills = ['geo-source-assets','geo-analysis','geo-content-production']; }
  else if (includesAny(hay, ['不知道','不会用','触发不到','路由','新手'])) { out.type = 'routing-update'; out.targetSkills = ['geo-workflow-hub','geo-hub','geo-troubleshooter']; }
  else out.targetSkills = ['geo-workflow-hub','geo-troubleshooter'];
  if (ctx.targetSkill) out.targetSkills = [ctx.targetSkill];
  if (!out.severity) {
    if (includesAny(hay, ['误判','错误结论','泄露','openkey','base url','发布url','publishedurl','p0'])) out.severity = 'P0';
...
function regressionTests(ctx, cls) {
  ...
  tests.push('rg -n "真实客户名|openKey|<LOCAL_PATH>|<SERVER_PATH>" geo-* || true');
