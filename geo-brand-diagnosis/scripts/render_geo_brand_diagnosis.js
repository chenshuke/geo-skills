...
const diagnosisMode=String(doc.meta['诊断模式']||reportType||title).toLowerCase();
const isProduct=/product|产品推荐|获客|推荐诊断/.test(diagnosisMode);
const isMixed=/mixed|综合|混合/.test(diagnosisMode);
const coverEn=isProduct?'Product Recommendation Report':isMixed?'GEO Diagnosis Report':'Brand Recognition Report';
const coverKicker=isProduct?'GEO · Product Recommendation Diagnosis':isMixed?'GEO · Combined Diagnosis':'GEO · Brand Recognition Diagnosis';
const defaultSummary=isProduct?'看清用户没有点名时，AI 是否提到、推荐并引用这个品牌。':isMixed?'分别看清 AI 的品牌认知和产品推荐结果。':'看清 AI 是否准确理解品牌身份、业务、优势、证据和边界。';
...
const html=`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><style>${css}</style></head><body><main class="page"><header class="cover"><div class="cover-top"><div class="brand-lockup"><div class="brand-mark">${esc(initials)}</div><div><span class="brand-name">${esc(brand)}</span><span class="brand-en">${esc(coverEn)}</span></div></div></div><div class="cover-main"><p class="cover-kicker">${esc(coverKicker)}</p><h1>${esc(title)}</h1><p class="cover-summary">${inline(doc.summary||defaultSummary)}</p></div><div class="cover-meta"><div class="meta-item"><span class="meta-label">报告类型</span><span class="meta-value">${esc(reportType)}</span></div><div class="meta-item"><span class="meta-label">测试范围</span><span class="meta-value">${esc(scope)}</span></div><div class="meta-item"><span class="meta-label">测试日期</span><span class="meta-value">${esc(testDate)}</span></div></div></header><article class="content">${md(doc.body)}</article></main></body></html>`;
